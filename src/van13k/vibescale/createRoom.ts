...
export function createRoom<TPlayer extends PlayerBase>(roomName: string, options: RoomOptions<TPlayer>): Room<TPlayer> {
  if (!roomName) {
  ...
  const stateChangeDetector = options.stateChangeDetectorFn
  ...
  const room: Room<TPlayer> = {
    ...
    mutatePlayer: (mutator) => {
      ...
      const newState = mutator(currentPlayer)
    ...
    connect() {
      const wsEndpoint = `${options.endpoint || 'https://vibescale.benallfree.com'}/${roomName}/websocket`
